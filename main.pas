...
type
  ...
  private
    ...
    //É possível declarar mais de uma variável do mesmo tipo, sendo na mesma line.
...
begin
  ...
  valor2 := StrToInt(txt_n2comp.text);
    if (valor1 > valor2) then
      begin
        ShowMessage('O valor 1 é maior que o valor 2.');
      end //quando eu tiver um else, esse 'end' não pode ter ';', o ';' é para fechar o 'if'.
      Else if (valor1 = valor2) then //'else if' acrescentando condições
      begin
        ShowMessage('Os valores são idênticos!');
      end
      Else if (valor1 < valor2) then
      begin
        ShowMessage('Os valor 1 é menor que o valor 2');
      end;
end;
...
begin
  ...
  media := (nota1+nota2) / 2;
  if (media >= 6) AND (media <= 10) OR (media=0)then //após '(media > 6)' exemplificando usos de 'and' e OR'.
  begin
    ShowMessage('A média é de ' + FloatToStr(media) + '' +'. Aprovado!');
  end
  else
  begin
    ShowMessage('A média é de ' + FloatToStr(media) + '' +'. Reprovado!' );
  end;
end;
...
begin
  nome := 'Sanches';
  sobrenome := 'Monteiro';
  endereco := 'Emílio Mori';

  //Após o nome do elemento (ex: txt_nome) vem ".{propriedade}", que pode ser checada
...
begin
//Para fazer a conversão de decimais, se usa StrTo{Float} / FloatToStr.
...
begin
  valor1 := StrToInt(txt_value.text); //.text -> por que é na propriedade texto.
  //Colocamos StrToInt para converter o valor String da caixa de texto para Int (formato objetivo), até
  //para podermos realizar operações como soma, que não são possíveis se recebesse uma string.
  valor2 := StrToInt(txt_value2.text);
  total := valor1+valor2;
  txt_value3.Text  := IntToStr(total);       //.text -> propriedade texto!
  //Propriedades como caption, text, só leêm elemetos em string, como precisamos
...
//Tipos de Variável:
  ...
  // REAL 5.00 -> NÚMERO REAL (POUCO UTILIZADO).
  // DOUBLE 100.50 -> NÚMERO DECIMAL (MAIS UTILIZADO).
  // BOOLEAM ATIVO := TRUE (OR FALSE) / booleano.

//Operadores (Usados em testes de {Condição}:
  // '<' Menor.
  // '>' Maior.
  // '=' Igual.
  // '<>' Diferente.
